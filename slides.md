...
```mermaid
flowchart LR
  A[MCP Client] <--> B[MCP Server (xiaohongshu_mcp.py)] --> C[Playwright 浏览器] --> D[小红书页面]
  B --> E[data/ 抓取结果]
  B --> F[browser_data/ 持久化会话]
```

**核心文件结构：**

- `xiaohongshu_mcp.py`：主服务入口，MCP 协议接口，负责登录、搜索、抓取、分析、评论发布
- `test_mcp.py`：异步测试脚本，示例调用接口
- `browser_data/`：浏览器持久化数据，扫码一次长期免登录
...
- `xiaohongshu_mcp.py`：主服务入口，推荐通过 MCP Client 调用 `--stdio` 启动
- `test_mcp.py`：异步测试脚本，示例 `login/get_note_content` 调用
...
- **页面抓取不到内容？**
  - 增加等待时间，检查网络与 User-Agent，清理 browser_data 后重试
- **Playwright 无法启动浏览器？**
